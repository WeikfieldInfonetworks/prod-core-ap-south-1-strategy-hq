...
class FPFSV4PE extends BaseStrategy {

    constructor() {
        super();
        this.name = 'Fifty Percent Full Spectrum V4 PE';
    ...
    setUserInfo(userName, userId) {
        ...
        this.strategyUtils.logStrategyInfo(`Fifty Percent Strategy v4 PE initialized for user: ${userName} (ID: ${userId})`);
    }
    ...
    initialize(globalDict, universalDict, blockDict, accessToken) {
        ...
        console.log('=== Fifty Percent Strategy v4 PE Initialization ===');
    ...
    processUpdateBlock(ticks) {
        ...
        for (const tick of ticks) {
            ...
            if (this.universalDict.peTokens.includes(token)) {
    ...
    async scenario1C(){
        ...
        instrument_1.buyPrice = ((parseFloat(this.buyPriceOnce) + this.prebuyBuyPriceTwice) / 2).toFixed(2);
        this.universalDict.instrumentMap[this.instrument_bought.token].buyPrice = ((parseFloat(this.buyPriceOnce) + this.prebuyBuyPriceTwice) / 2).toFixed(2);
        let buyResult = null;
        try {
            buyResult = await this.buyInstrument(instrument_1);
            if (buyResult && buyResult.success) {
            ...
            this.rebuyDone = true;
            this.rebuyPrice = this.prebuyBuyPriceTwice;
            this.rebuyAveragePrice = ((parseFloat(this.buyPriceOnce) + this.prebuyBuyPriceTwice) / 2).toFixed(2);
