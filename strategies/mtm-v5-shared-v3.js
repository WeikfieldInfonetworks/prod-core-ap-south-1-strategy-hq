...
class MTMV5SharedStrategyV3 extends BaseStrategy {
    constructor() {
        super();
    ...
    setUserInfo(userName, userId) {
        ...
        this.strategyUtils.logStrategyInfo(`MTM V5 Strategy Shared V3 initialized for user: ${userName} (ID: ${userId})`);
        this.userId = userId;
    }
    ...
    initialize(globalDict, universalDict, blockDict, accessToken) {
        ...
        console.log('=== MTM V5 Strategy Shared V3 Initialization ===');
        ...
        if(!this.universalDict.usePrebuy){
            ...
            if (filter_x && !this.prebuyHit){
                ...
                if(!this.repetition.observed){
                    ...
                    let other_instrument = real_instrument.symbol.includes('CE') ? closestPEto200 : closestCEto200;
                    this.universalDict.instrumentMap[other_instrument.token.toString()].buyPrice = parseFloat(other_instrument.last);
                }
                ...
                try {
                    const buyResult = await this.buyInstrument(real_instrument, "FIRST_BUY");
                    ...
                    this.universalDict.instrumentMap[this.prebuyBoughtToken].last = this.prebuyBuyPriceOnce;
                ...
                if(this.instrumentSet.length > 0){
                    try {
                        await this.executeInstrumentSet('buy', "FIRST_BUY");
                    }
                    catch(error){
                ...
                this.announceRealPrice(this.universalDict.instrumentMap[this.prebuyBoughtToken]);
                ...
                //     this.savedState['quantity'] = this.lockedQuantity;
                // }
    ...
    async processDiff10Block(ticks) {
        ...
        console.log(`TARGET: ${this.universalDict.target}, STOPLOSS: ${this.globalDict.stoploss}, QUANTITY: ${this.lockedQuantity}`);
        ...
        if(!this.targetNet && this.universalDict.usePrebuy && this.actualRebuyDone){
            this.targetNet = (instrument_1.last - instrument_1.buyPrice) >= (((this.universalDict.target - this.universalDict.residual)/2) - 0.0); // Casting net if price within 1 point of target
            if(this.targetNet){
        ...
        // SL2A OBSERVER.
        if(this.actualRebuyDone && ((instrument_1.last - instrument_1.buyPrice) >= (0.75*((this.universalDict.target - this.universalDict.residual)/2))) && !this.sl2a && this.exit_at_stoploss){
        ...
        if(this.entry_7){
            ...
            if(!this.universalDict.usePrebuy){
                ...
                try {
                    sellResult = await this.sellInstrument(instrument_1, "TARGET_SELL");
                    if (sellResult && sellResult.success) {
                ...
                if(this.instrumentSet.length > 0){
                    try {
                        await this.executeInstrumentSet('sell', "TARGET_SELL");
                    }
                    catch(error){
                        this.strategyUtils.logStrategyError(`Error executing instrument set: ${error.message}`);
                    }
        ...
        if(this.universalDict.usePrebuy && !this.entry_7 && !this.lockScenario){
            ...
            if(this.shouldPlayScenario1EA()){
                await this.scenario1EA();
            }
            ...
            if(this.shouldPlayScenarioSL5()){
                await this.scenarioSL5();
            }
            if(this.shouldPlayScenarioSL5A()){
                await this.scenarioSL5A();
            }
        ...
        if (this.boughtSold) {
            ...
            if(!this.completeTransaction){
                await this.sellInstrument(instrument_1, "INCOMPLETE_TRX_SELL");
                let sellResult = null;
                let diff = 0;
                try {
                    if (sellResult && sellResult.success) {
                ...
                if(this.instrumentSet.length > 0){
                    try {
                        await this.executeInstrumentSet('sell', "INCOMPLETE_TRX_SELL");
                    }
                    catch(error){
                        this.strategyUtils.logStrategyError(`Error executing instrument set: ${error.message}`);
                    }
    ...
    async scenario1C(){
        ...
        if(!this.universalDict.useOppositeStrategy){
            ...
            let buyResult = null;
            try {
                buyResult = await this.buyInstrument(instrument_1, "REBUY");
                if (buyResult && buyResult.success) {
                ...
                if(this.instrumentSet.length > 0){
                    try {
                        await this.executeInstrumentSet('buy', "REBUY");
                    }
                    catch(error){
                        this.strategyUtils.logStrategyError(`Error executing instrument set: ${error.message}`);
                    }
                ...
                this.prebuyBuyPriceTwice = buyResult.executedPrice == 0 ? instrument_1.last : buyResult.executedPrice;
                this.universalDict.instrumentMap[this.prebuyBoughtToken].last = this.prebuyBuyPriceTwice;
            ...
            let sellResult = null;
            try {
                sellResult = await this.sellInstrument(instrument_1, "SELL_WHEN_REBUY");
                if (sellResult && sellResult.success) {
                    this.strategyUtils.logStrategyInfo(`Real instrument sold - Executed price: ${sellResult.executedPrice}`);
                }
                if(this.instrumentSet.length > 0){
                    try {
                        await this.executeInstrumentSet('sell', "SELL_WHEN_REBUY");
                    }
                    catch(error){
                        this.strategyUtils.logStrategyError(`Error executing instrument set: ${error.message}`);
                    }
                ...
                const executedSellPrice = (sellResult && sellResult.executedPrice && sellResult.executedPrice != 0)
                    ...
                    : instrument_1.last;
                this.universalDict.instrumentMap[this.prebuyBoughtToken].last = executedSellPrice;
                this.announceRealPrice(this.universalDict.instrumentMap[this.prebuyBoughtToken]);
    ...
    async scenario1E(){
        ...
        instrument_1.buyPrice = instrument_1.last;
        this.universalDict.instrumentMap[this.prebuyBoughtToken].buyPrice = instrument_1.last;
        let buyResult = null;
        try {
            buyResult = await this.buyInstrument(instrument_1);
            if (buyResult && buyResult.success) {
            ...
            instrument_1.buyPrice = buyResult.executedPrice == 0 ? instrument_1.last : buyResult.executedPrice;
        ...
        // this.emitResidual();
        // this.strategyUtils.logStrategyInfo(`DIFF AFTER 1E: ${diff_val}`);
    ...
    async scenarioSL5(){
        ...
        this.strategyUtils.logStrategyInfo(`Scenario SL5 in action.`)
        //SELL
        // this.strategyUtils.logStrategyInfo('Selling existing instrument and buying opposite.');
        // this.realBuyStoplossHit = true;
        let sellResult = null;
    ...
    async scenarioSL5A(){
        ...
        this.strategyUtils.logStrategyInfo(`Scenario SL5A in action.`)
    ...
    shouldPlayScenario1DA(){
        ...
        return flag && !this.boughtSold && this.actualRebuyDone && !this.scenario1DAdone && !this._1DAHit && this.universalDict.cycles == 1 && false;
    }
    ...
    shouldPlayScenario1EA(){
        return this.scenario1ea_hit && !this.scenario1EAdone && !this.boughtSold && !this.targetHit && this.scenario1Cdone && false;
    }
    ...
    shouldPlayScenarioSL2(){
        let instrument_1 = this.universalDict.instrumentMap[this.prebuyBoughtToken];
        return (instrument_1.last <= instrument_1.buyPrice) && this.exit_at_stoploss && !this.boughtSold && !this.scenario1Adone && !this.scenario1Bdone && !this.afterTarget && !this.scenarioSL4Done; 
    }
    ...
    shouldPlayScenarioSL3(){
        return this.targetHit && !this.boughtSold && !this.afterTarget && !this.scenarioSL4Done && false;
    }
    ...
    shouldPlayScenarioSL5(){
        let instrument_1 = this.universalDict.instrumentMap[this.prebuyBoughtToken];
        return (instrument_1.last - instrument_1.buyPrice) <= (-1*this.universalDict.rebuyAt) && !this.boughtSold && !this.afterTarget && !this.rebuyFound && !this.scenarioSL5Done && !this.sl5aHit && this.universalDict.exitAtNegativeRebuy && !this.scenarioSL4Done;
    }
    ...
    shouldPlayScenarioSL5A(){
        ...
        if(this.scenario1Cdone){
            return (diff <= Math.floor(parseFloat(this.prebuyBuyPriceOnce)+parseFloat(this.universalDict.rebuyAt)/2)) && !this.boughtSold && !this.afterTarget && this.sl5aHit && !this.scenarioSL5ADone && false;
        }
        // else {
        ...
        try {
            if (tradingEnabled) {
                ...
                if (boughtOrderResult.success) {
                    ...
                    this.strategyUtils.logOrderPlaced('buy', boughtInstrument.symbol, boughtInstrument.last, this.lockedQuantity || 65, this.boughtToken);
                } else {
                    this.strategyUtils.logStrategyError(`Failed to place buy order for ${boughtInstrument.symbol}: ${boughtOrderResult.error}`);
                    this.strategyUtils.logOrderFailed('buy', boughtInstrument.symbol, boughtInstrument.last, this.lockedQuantity || 65, this.boughtToken, boughtOrderResult.error);
                }
                ...
                if (oppOrderResult.success) {
                    ...
                    this.strategyUtils.logOrderPlaced('buy', oppInstrument.symbol, oppInstrument.last, this.lockedQuantity || 65, this.oppBoughtToken);
                } else {
                    this.strategyUtils.logStrategyError(`Failed to place buy order for ${oppInstrument.symbol}: ${oppOrderResult.error}`);
                    this.strategyUtils.logOrderFailed('buy', oppInstrument.symbol, oppInstrument.last, this.lockedQuantity || 65, this.oppBoughtToken, oppOrderResult.error);
                }
            ...
            this.strategyUtils.logStrategyInfo(`Total investment: ${(boughtInstrument.last + oppInstrument.last) * (this.lockedQuantity || 65)}`);
            ...
            const preboughtData = {
                ...
                ceInstrument: {
                    ...
                    quantity: this.lockedQuantity || 65,
    ...
    getUniversalDictParameters() {
        return {
            ...
            disableSecondTrade: {
                type: 'boolean',
                default: false,
                description: 'Disable second trade'
            },
            enableTrading: {
                type: 'boolean',
                default: false,
        ...
        try {
            if (tradingEnabled) {
                ...
                if (sellResult1.success) {
                    ...
                    this.strategyUtils.logOrderPlaced('sell', instrument1.symbol, instrument1.last, this.lockedQuantity || 65, instrument1.token);
                ...
                if (sellResult2.success) {
                    ...
                    this.strategyUtils.logOrderPlaced('sell', instrument2.symbol, instrument2.last, this.lockedQuantity || 65, instrument2.token);
    ...
    async sellInstrument(instrument, tag) {
        ...
        try {
            ...
            this.emitSimpleTradeEvent('sell', instrument.symbol, executedPrice != 0 ? executedPrice : instrument.last, this.lockedQuantity || 65, tag);
    ...
    async buyInstrument(instrument, tag) {
        ...
        try {
            ...
            // Emit simplified trade event after determining executed price
            this.emitSimpleTradeEvent('buy', instrument.symbol, executedPrice != 0 ? executedPrice : instrument.last, this.lockedQuantity || 65, tag);
    ...
    emitSimpleTradeEvent(action, symbol, price, quantity, tag) {
        const tradeEvent = {
            action, // 'buy' or 'sell'
            tag,
            symbol,
        ...
        corpusArray.forEach(line => {
            ...
            if(parseInt(cycle) === parseInt(this.universalDict.cycles) && state === 'COMPLETE'){
                if(pairIDlist.includes(pairID)){
                    ...
                    if(pairID != this.getPairID(this.userId)){
                        this.boughtSold = true;
                    }
                }
            }
    ...
    announceRealPrice(instrument_1, instrument_2 = null){
        this.bookmark++;
        if(instrument_2){
            this.appendToGalacticOutput(`${this.getPairID(this.userId)}|${this.universalDict.cycles}|REAL_PRICES|${this.bookmark}|${JSON.stringify({[instrument_1.symbol] : instrument_1.last, [instrument_2.symbol]: instrument_2.last})}\n`);
        }
        else{
            this.appendToGalacticOutput(`${this.getPairID(this.userId)}|${this.universalDict.cycles}|REAL_PRICES|${this.bookmark}|${JSON.stringify({[instrument_1.symbol] : instrument_1.last})}\n`);
        }
    }
    ...
    checkBookMark(){
        let corpus = fs.readFileSync("output/galactic.txt", 'utf8');
        let corpusArray = corpus.split('\n');
        let latestBookmark = this.bookmark;
        ...
        corpusArray.forEach(line => {
            ...
            let [pairID, cycle, state, incomingBookmark, data] = line.split('|');
            if(
                parseInt(cycle) === parseInt(this.universalDict.cycles) &&
                state === 'REAL_PRICES' &&
                pairID === this.getPairID(this.userId)
            ){
                let parsedBookmark = parseInt(incomingBookmark);
                if(!Number.isNaN(parsedBookmark) && parsedBookmark > latestBookmark){
                    latestBookmark = parsedBookmark;
                }
            }
        });

        this.bookmark = latestBookmark;
    }
    ...
    checkPartnerInstanceSet(){
        ...
        corpusArray.forEach(line => {
            ...
            if(userId == id_list[1]){
                ...
                else if(parseInt(cycle) === parseInt(this.universalDict.cycles) && state === 'MTM_HIT' && !this.mtmHit && !this.announcementDone){
                    this.mtmHit = true;
    ...
    announceTargetHit(){        this.sl5aHit = true;
    ...
    announceMTMHit(){
        this.appendToGlobalOutput(`${this.universalDict.cycles}:${this.userId}:${this.cycleInstanceId}:MTM_HIT\n`);
        this.announcementDone = true;
    }
    ...
    getCommonParameters(){
        let params = ["enableTrading", "enableTradingForNextCycle", "enableManualEntry", "enterNow", "peakDefInCurrentCycle", "peakDefAfterFirstCycle", "quantity", "target", "rebuyAt", "exitAtFirstBuy", "exitAtNegativeRebuy", "enableExitAfterRebuy", "mtmTarget", "enableMTM", "goingLiveInFirstCycle", "useOppositeStrategy", "extraPairs", "disableSecondTrade"];
        return params;
    }
    ...
    async executeInstrumentSet(order_type, tag){
        try{
            if(order_type === 'buy'){
                for(let instrument of this.instrumentSet){
                    await this.buyInstrument(instrument, tag);
                }
            }
            else if(order_type === 'sell'){
                for(let instrument of this.instrumentSet){
                    await this.sellInstrument(instrument, tag);
                }
            }
        }
        catch(error){
